...
public class Answer {
  public static double[, ] CreateRandomMatrix(int m, int n, int minLimitRandom, int maxLimitRandom) {
    ...
    for (int i = 0; i < m; i++){
        for(int j = 0; j < n; j++){
            //Заполнение массива числами преобразованными из целого цисла в вещественное
            matrix[i, j] = Convert.ToDouble(new Random().Next(minLimitRandom, maxLimitRandom)) / 10;
        }
    }
    return matrix;
  }
  ...
  public static void Main(string[] args) {
    ...
    if (args.Length >= 4) {
      m = int.Parse(args[0]);  // преобразуем аргумент с индексом 0 в целое число и присваеваем его m
      n = int.Parse(args[1]);  // преобразуем аргумент с индексом 1 в целое число и присваеваем его n
